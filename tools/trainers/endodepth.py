...
class plEndoDepth(pl.LightningModule):
    ...
    def __init__(self, options=None, test=None,frozen_stages=-1, *args, **kwargs):
        ...
        elif options.backbone == 'resnet':
            self.encoder = ResnetAttentionEncoder(options.num_layers, False)
            self.num_ch_enc = self.encoder.num_ch_enc
        else:
            raise ValueError(f"Invalid backbone: {options.backbone}. Must be 'transformer' or 'resnet'")
    ...
    def compute_loss(self, images: torch.Tensor, depth_gt: torch.Tensor, depth: torch.Tensor, prefix: str = ""):
        ...
        if self.options.use_smooth_loss:
            ...
            # Resize images to match depth size for smoothness loss
            if images.shape[-2:] != depth.shape[-2:]:
                images_resized = F.interpolate(images, size=depth.shape[-2:], mode='bilinear', align_corners=False)
            else:
                images_resized = images
            losses[f'{prefix}loss/smooth'] = inverse_depth_smoothness_loss(idepth, images_resized)
        else:
